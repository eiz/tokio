...
pub(crate) type ChildStdin = PollSource<NamedPipe>;
pub(crate) type ChildStdout = PollSource<NamedPipe>;
pub(crate) type ChildStderr = PollSource<NamedPipe>;

fn stdio<T>(option: Option<T>) -> Option<PollSource<NamedPipe>>
where
    T: IntoRawHandle,
{
    let io = match option {
